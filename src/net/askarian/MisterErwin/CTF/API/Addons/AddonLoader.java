...
//						+ a.getAuthors().get(0));
		...
		if (Addons.size() == 0) {
			plugin.getServer().getConsoleSender().sendMessage(ChatColor.AQUA + "[CTF] No addons were loaded ... Maybe you onna download some!");
			plugin.getServer().getConsoleSender().sendMessage(ChatColor.AQUA + "[CTF] Just look into the forum @ dev.bukkit!!!");
