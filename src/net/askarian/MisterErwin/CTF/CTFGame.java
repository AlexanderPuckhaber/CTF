...
//	public HashMap<?, ?> bleeds = new HashMap<Object, Object>();
	...
	public void startGame() {
		if (this.SpawnA.getWorld() == null || buildonly == true){
			running = false;
			idle = true;
			after = false;
...
//				plugin.tm.setClass(p, ite[1]);
			...
			if (plugin.tm.getTeam(p) == "A")
				p.teleport(plugin.Game.SpawnA, PlayerTeleportEvent.TeleportCause.PLUGIN);
			else if (plugin.tm.getTeam(p) == "B")
				p.teleport(plugin.Game.SpawnB, PlayerTeleportEvent.TeleportCause.PLUGIN);
...
//				lastteam= "A";
	...
	public void saveFlagLocationA(Location loc) {
		if (!new File(""+loc.getWorld().getName()+"/level.dat").exists()){
			plugin.log.warning("[CTF]Error while flaglocation saving => World not found");
			return;
		}
		File fs = new File(""+loc.getWorld().getName()+"/flaglocA.ctf");
		WriteLocFile(fs, loc.getX(), loc.getY(), loc.getZ(), loc.getYaw(), loc.getPitch());			
		plugin.Game.Spawn = loc;
	}
	
	public void saveFlagLocationB(Location loc) {
		if (!new File(""+loc.getWorld().getName()+"/level.dat").exists()){
			plugin.log.warning("[CTF]Error while spawnlocation saving => World not found");
			return;
		}
...
//###################LOAD LOCATIONS###############
	...
	public void WriteLocFile(File f, double X, double Y, double Z, float yaw, float pitch){
		if (!f.exists()){
			try {
		      File oldFile = f;
		      ...
		      plugin.log.warning("[CTF] Error while writing location file:" + f.getAbsolutePath());
		    }
		    return;
		}//Ende Create
	...
	public void start(CommandSender p, boolean vote, boolean random) {
	    ...
	    if (zl == 0|| Maps.get(0)==null){
	    	p.sendMessage(ChatColor.RED + "No Maps found in folder " + folder.getName());
	    	return;
	    }
	    if (random){
...
//	    	}
	...
	public void tp(Player p) {
		if (p == null || p.isDead())
			return;
		if (this.Spawn == null) {
			p.teleport(this.Spawn.getWorld().getSpawnLocation(), PlayerTeleportEvent.TeleportCause.PLUGIN);
		}else {
	...
	public void loadConfig (String worldname) {
		...
		try {
		    ...
		    World w = plugin.getServer().getWorld(worldname);
		    if (w == null) {
		    	plugin.log.warning("[CTF] Error while loading World config : null");
		    	return;
		    }
		    //classes
//		    this.disallowedclasses = getListbyString(snr.nextLine().trim().split("=")[1]);
		    this.freeclasses = getListbyString(snr.nextLine().trim().split("=")[1]);
		    this.disallowedclasses = getListbyString(snr.nextLine().trim().split("=")[1]);
		    //time & game
		    this.pretime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    this.posttime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    this.gametime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    this.resettime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    this.foundtime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    this.MaxCaptures = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    //Sponges
		    this.hboost = Double.parseDouble(snr.nextLine().trim().split("=")[1]);
		    this.vboost = Double.parseDouble(snr.nextLine().trim().split("=")[1]);
		    //worldtime
		    this.starttimeticks = Long.parseLong(snr.nextLine().trim().split("=")[1]);
		    this.holdtime = Integer.parseInt(snr.nextLine().trim().split("=")[1]);
		    //Gamemode
...
//			pw.write("#allowedclasses={soldier,archer,assassin}\r\n");
			...
			plugin.log.warning("[CTF] Can't create World-Config! " + f.getAbsolutePath());
		}
	}
...
//		plugin.getServer().broadcastMessage("GAME ENDS - " + plugin.Game.posttime);
...
//			plugin.cm.SendMessage(p, ChatColor.GREEN + "Don't forgot to vote via /vote");
		...
		if (plugin.Game.TeamACaptures > plugin.Game.TeamBCaptures)
		{
			if (plugin.Game.SpawnA != null)
				for (Map.Entry<String, String> entry : plugin.tm.TeamB.entrySet()) 
					if (entry != null && plugin.getServer().getPlayer(entry.getKey()) != null)
						plugin.Game.SpawnA.getWorld().strikeLightning(plugin.getServer().getPlayer(entry.getKey()).getLocation());
				
		}else if (plugin.Game.TeamACaptures < plugin.Game.TeamBCaptures)
		{
			if (plugin.Game.SpawnA != null)
				for (Map.Entry<String, String> entry : plugin.tm.TeamA.entrySet()) 
					if (entry != null && plugin.getServer().getPlayer(entry.getKey()) != null)
						plugin.Game.SpawnA.getWorld().strikeLightning(plugin.getServer().getPlayer(entry.getKey()).getLocation());
