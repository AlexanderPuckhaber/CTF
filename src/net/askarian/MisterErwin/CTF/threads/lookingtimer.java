...
//		}
		...
		//Addon Events
		if (plugin.Game.idle)
			plugin.getServer().getPluginManager().callEvent(new CTFPreTickEvent(plugin.Game.time2start));
		else if (plugin.Game.running)
			plugin.getServer().getPluginManager().callEvent(new CTFInTickEvent(plugin.Game.time2play));
		else if (plugin.Game.after)
			plugin.getServer().getPluginManager().callEvent(new CTFPostTickEvent(plugin.Game.time2leave));

		
		//Exist Flag?
		boolean FlagAexists = false, FlagBexists = false;
		
		if (plugin.Game.FlagAHolder instanceof String && plugin.Game.FlagAHolder == "Home" && plugin.Game.running){
//			for (org.bukkit.entity.Entity i : plugin.Game.SpawnA.getWorld().getEntities())
//				if (i instanceof Item && ((Item)i).getUniqueId().toString() == plugin.Game.FlagItemA.toString()){
//					FlagAexists = true;
//					continue;
//				}
		...
		if (plugin.Game.FlagBHolder instanceof String && plugin.Game.FlagBHolder == "Home" && plugin.Game.running){
//			for (org.bukkit.entity.Entity i : plugin.Game.SpawnB.getWorld().getEntities())
//				if (i instanceof Item && ((Item)i).getUniqueId().toString() == plugin.Game.FlagItemB.toString()){
//					FlagBexists = true;
//					continue;
//				}
		...
		if (plugin.Game.FlagAReset > 0) {
			/*if (FlagAexists && plugin.Game.FlagAHolder != "reset")
				plugin.Game.FlagAReset = 0;				
			else {*/
				plugin.Game.FlagAReset--;
				if (plugin.Game.FlagAReset == 0){
					plugin.cm.allMessage(ChatColor.YELLOW + "The " + plugin.conm.get("TeamNameA") + ChatColor.YELLOW + " flag has been restored!" );
					plugin.ctfu.reset("A");
				}
			//}
		}
		...
		if (plugin.Game.FlagBReset > 0) {
			/*if (FlagBexists  && plugin.Game.FlagBHolder != "reset")
				plugin.Game.FlagBReset = 0;
			else {*/
				plugin.Game.FlagBReset--;
				if (plugin.Game.FlagBReset == 0){
					plugin.cm.allMessage(ChatColor.YELLOW + "The " + plugin.conm.get("TeamNameB") + ChatColor.YELLOW + " flag has been restored!" );
					plugin.ctfu.reset("B");
				}
			//}
		}
		//Spawn A
		Iterator<String> it;
		if (plugin.Game.spawnPlayersA != null && plugin.Game.spawnPlayersA.size() > 0){

			it = plugin.Game.spawnPlayersA.iterator();
			while (it.hasNext()) {
				Player p = plugin.getServer().getPlayerExact(it.next().toString());
		...
		//Spawn B
		if (plugin.Game.spawnPlayersB != null && plugin.Game.spawnPlayersB.size() > 0){
		...
		if ( plugin.Game.idle != true){
			...
			if (plugin.Game.waitingPlayers.size() >= Integer.valueOf( plugin.conm.get("player2start")) && plugin.Game.buildonly == false){
				plugin.Game.time2start--;
				plugin.Game.premessage = "";
			}else{
				if (!plugin.Game.buildonly ){
					plugin.Game.premessage = ChatColor.RED + " " + (Integer.valueOf( plugin.conm.get("player2start")) - plugin.Game.waitingPlayers.size()) + " more Player!";
					...
					if (plugin.Game.time2start * 2 < plugin.Game.pretime){
						...
						for (String pn : plugin.Game.waitingPlayers.keySet()){
							try{
								if (plugin.getServer().getPlayerExact(pn).hasPermission("CTF.admin") == true){
									OPinGame = true;
									break;
								}
								continue;
...
//			plugin.ctfu.playNote(Sound.NOTE_PLING, 10, 1);
		...
		if (plugin.Game.time2play == 60)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 60 sec.!");
		if (plugin.Game.time2play == 30)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 30 sec.!");
		if (plugin.Game.time2play == 15)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 15 sec.!");
		if (plugin.Game.time2play == 10)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 10 sec.! - Hurry up!");
		if (plugin.Game.time2play == 5)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 5 sec.! - Hurry up!");
		if (plugin.Game.time2play == 4)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 4 sec.! - Hurry up!");
		if (plugin.Game.time2play == 3)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 3 sec.! - Hurry up!");
		if (plugin.Game.time2play == 2)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 2 sec.! - Hurry up!");
		if (plugin.Game.time2play == 1)
			plugin.cm.allMessage(ChatColor.DARK_PURPLE + "Game ends in 1 sec.!");
		if (plugin.Game.time2play <= 0)
			plugin.Game.stopCauseCounter();
		//Play Sound
		if (plugin.Game.time2play < 4){
			if (plugin.Game.time2play < 1)
				plugin.ctfu.playNote(Sound.NOTE_PLING, 9, 1);
			else
				plugin.ctfu.playNote(Sound.NOTE_BASS_GUITAR, 9, 1);
		}
		...
		//Halt time
		if (plugin.Game.holdtime == 1 && plugin.Game.Spawn != null && plugin.Game.Spawn.getWorld() != null){
		...
		if (plugin.Game.running ){
			if (!FlagAexists && plugin.Game.FlagAHolder == "Home"){
				if (F2ndA){
					F2ndA = false;
					plugin.log.info("Flag A 'll reset" + plugin.Game.FlagAHolder);
			...
			if (!FlagBexists && plugin.Game.FlagBHolder == "Home" ){
				if (F2ndB) {
					F2ndA = false;
					plugin.log.info("Flag B 'll reset" + plugin.Game.FlagBHolder);
