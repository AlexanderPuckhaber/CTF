...
import net.askarian.MisterErwin.CTF.ChatManager.ChatMode;
 ...
 * 
 * And if you made a few cool changes, just send them to me and I'll propably implement them into the plugin
 * 
 * If you've modify CTF, you do this on your OWN RISK!
 * 
 * If you onna resell my work, give me at least credits(not only in the source!) and send me a PM via bukkit...
 * 
 * @author MisterErwin
 */
...
public class CTF extends JavaPlugin {
  public Logger log;
	...
	public void onEnable() {
		...
		trans.register("avaibleclasses", ChatColor.WHITE+ "AVAIBLE CLASSES - Avaible for everyone!");
		trans.register("premiumclasses",  ChatColor.GREEN+ "PREMIUM CLASSES - Order @ ...");
		trans.register("buythatclass",  ChatColor.RED + "&k is a premium class - Order @ ...");
//		trans.register("youareinTeam", ChatColor.GRAY + "You are in Team ");
		trans.register("AWon", ChatColor.GOLD + "Team : " + ChatColor.RED + "Red" + ChatColor.GOLD + " won the Game!!!");
		trans.register("BWon", ChatColor.GOLD + "Team : " + ChatColor.BLUE + "Blue" + ChatColor.GOLD + " won the Game!!!");
		trans.register("NoWon", ChatColor.GOLD + "No Team won the Game - Time was over!");
		...
		trans.register("Bcaptured", ChatColor.BLUE + "&P" + ChatColor.YELLOW + " has captured the " + ChatColor.RED + "Red" + ChatColor.YELLOW + "Team's flag!");

		trans.register("Astolen", "&P" + ChatColor.YELLOW + " has stolen the " + ChatColor.RED  + "Red"  + ChatColor.YELLOW + "Team's flag!");
		trans.register("Bstolen", "&P" + ChatColor.YELLOW + " has stolen the " + ChatColor.BLUE + "Blue" + ChatColor.YELLOW + "Team's flag!");

		trans.register("YouarenowA", "You are now a &k");
		trans.register("YouarenowAN", "You are now an &k");

		trans.register("remember2vote", ChatColor.GREEN + "Don't forgot to vote via /vote");
		...
		this.cm.cm = ChatMode.valueOf(conm.get("ChatMode"));
...
//		this.InvM.load();
		...
		this.getServer().getConsoleSender().sendMessage(ChatColor.YELLOW + "If you found a bug, report it: http://dev.bukkit.org/server-mods/ctf/tickets/");
...
//		getCommand("class").setTabCompleter(this);
		...
		if (Boolean.valueOf(this.conm.get("auto")) == true) {
			this.getServer().getConsoleSender().sendMessage(ChatColor.BLUE + "CTF is now in automatic mode!");
	...
	public void registerHooks() {
		...
		getServer().getPluginManager()
		.registerEvents(new TrackerListener(this), this);

		this.BCID = getServer().getScheduler().scheduleSyncRepeatingTask(this,
				new BroadcasterThread(this), 60L, Long.valueOf(this.conm.get("broadcastertime")));
		if (this.cm.cm == ChatMode.HUD || this.cm.cm == ChatMode.SCOREBOARD ||  this.cm.cm == ChatMode.CHATBOARD ){
			this.CHID = getServer().getScheduler().scheduleSyncRepeatingTask(this,
				new HoldChatThread(this), 60L, 20L);
