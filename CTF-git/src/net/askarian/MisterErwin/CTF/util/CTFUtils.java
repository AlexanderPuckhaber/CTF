...
public class CTFUtils {
  private CTF plugin;
	...
	private int[] fences = {85, 113, Material.IRON_FENCE.getId()}; //Fence,Netherfence, Ironfence
...
//			loc = loc;
//		e = loc.getWorld().dropItem(loc, new ItemStack(Material.WOOL, 1, color.getData())).setVelocity(new Vector());
		loc.setY( loc.getY() + 1 );

		Entity e = loc.getWorld().dropItem(loc, new ItemStack(Material.WOOL, 1, color.getWoolData()));
...
//				plugin.log.info("Flag placed NOT at " + loc.toString());
				...
				if (color == DyeColor.RED)
					e.setMetadata("redflag", new FixedMetadataValue(plugin, e.getEntityId()));
				
				if (color == DyeColor.BLUE)
					e.setMetadata("blueflag", new FixedMetadataValue(plugin, e.getEntityId()));
		return (Item) e;
	}

	public void saveFlagA(Player p) {
		Block b = p.getLocation().getBlock().getRelative(BlockFace.DOWN);
		if (b.getType() != Material.FENCE && b.getType() != Material.NETHER_FENCE && b.getType() != Material.IRON_FENCE) {
	...
	public boolean isfence (Block b) {
		return (b.getType() == Material.FENCE || b.getType() == Material.NETHER_FENCE ||b.getType() == Material.IRON_FENCE);
	}
	...
	public void pickup (Player p, String teamsflag) {
		if (teamsflag == "A" ) {
			...
			if (plugin.tm.getTeam(p) == "A") {
				plugin.cm.allMessage(ChatColor.RED + p.getName() + ChatColor.DARK_PURPLE + " recaptured the " + ChatColor.RED + "Red " + ChatColor.DARK_PURPLE + "Flag!");
				reset ("A");
			...
			plugin.Game.FlagItemAW = SpawnFlag(plugin.Game.getFlagA(), DyeColor.WHITE);
			plugin.cm.allMessage(plugin.trans.get("Astolen").replace("&P", p.getDisplayName()));
			plugin.Game.getFlagA().getWorld().strikeLightningEffect(plugin.Game.getFlagA());
			...
			if (plugin.tm.getTeam(p) == "B") {
				plugin.cm.allMessage(ChatColor.BLUE + p.getName() + ChatColor.DARK_PURPLE + " recaptured the " + ChatColor.BLUE + "Blue " + ChatColor.DARK_PURPLE + "Flag!");
				reset ("B");
				plugin.tracker.addScore(p.getName(), ScoreType.Recapture);
				return;
			}
			...
			plugin.Game.FlagItemBW = SpawnFlag(plugin.Game.getFlagB(), DyeColor.WHITE);
			plugin.cm.allMessage(plugin.trans.get("Bstolen").replace("&P", p.getDisplayName()));
			plugin.Game.getFlagB().getWorld().strikeLightningEffect(plugin.Game.getFlagB());
	...
	public void die(Player p, String team) {
		if (team == "A") {
			drop (p, p.getWorld().dropItemNaturally(p.getLocation(), new ItemStack(Material.WOOL, 1, DyeColor.RED.getData())));
		}else if (team == "B") {
			drop (p, p.getWorld().dropItemNaturally(p.getLocation(), new ItemStack(Material.WOOL, 1, DyeColor.BLUE.getData())));
		}
	...
	public void reset (String team) {
		if (team == "A"){
			if (plugin.Game.FlagItemAW != null && !plugin.Game.FlagItemAW.isDead()) {
				for (Entity e : plugin.Game.getFlagA().getWorld().getEntities()) {
					if (!(e instanceof Item))
						continue;
					if (plugin.Game.FlagItemAW != null && e.getUniqueId() == plugin.Game.FlagItemAW.getUniqueId()){
						plugin.Game.FlagItemA = SpawnFlag(plugin.Game.getFlagA(), DyeColor.RED);
						e.remove();
			...
			if (plugin.Game.FlagItemBW != null && !plugin.Game.FlagItemBW.isDead()) {
				for (Entity e : plugin.Game.getFlagB().getWorld().getEntities()) {
					if (!(e instanceof Item))
						continue;
					if ( plugin.Game.FlagItemBW != null && e.getUniqueId() == plugin.Game.FlagItemBW.getUniqueId()){
						plugin.Game.FlagItemB = SpawnFlag(plugin.Game.getFlagB(), DyeColor.BLUE);
						e.remove();
	...
	public void captured(Player p, String team) {
		...
		if (team == "A"){
			...
			plugin.cm.allMessage(plugin.trans.get("Acaptured").replace("&P", p.getDisplayName()));
			plugin.cm.allupdate();
			reset ("B");
			...
			plugin.cm.allMessage(plugin.trans.get("Bcaptured").replace("&P", p.getDisplayName()));
			plugin.cm.allupdate();
			reset ("A");
