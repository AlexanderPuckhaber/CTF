...
public class ClassManager {
	...
	public ClassManager (CTF pl){
		plugin = pl;
		this.Classes.addAll(new ExClassLoader(this.plugin).load(this.plugin.getDataFolder()
...
//			y = 4*9;
			...
			plugin.imm.create(p, "Class Selector", x, new IconMenu.OptionClickEventHandler() {
                @Override
                public void onOptionClick(IconMenu.OptionClickEvent event) {
//                    event.getPlayer().sendMessage("You have chosen " + event.getName());
                    plugin.comm.changeclass(event.getPlayer(), event.getName());
			...
			for (CtFClass cl : this.Classes){
				if (cl == null || cl.getIcon()== null ||cl.getDescription() == null ||cl.getName() == null || cl.getCommand() == null)
					plugin.log.info("ERROR");
				else{
					if (!hasperm(p, cl.getCommand())){
						noperm.add(cl);
					}else{
//						plugin.log.info(i + " " + cl.getIcon()+ " " + cl.getName()+ " " + cl.getCommand()+ " " + cl.getDescription());
						plugin.imm.setOption(p, i, cl.getIcon(), cl.getName(), cl.getCommand(), cl.getDescription());
						i++;
					}
				}
			}
			...
			for (CtFClass cl : noperm){
				if (cl == null || cl.getIcon()== null ||cl.getDescription() == null ||cl.getName() == null || cl.getCommand() == null)
					...
					plugin.imm.setOption(p, i, cl.getIcon(), ChatColor.RED + cl.getName(), cl.getCommand(), cl.getDescription());
					i++;
...
//            .setOption(5, new ItemStack(Material.EMERALD, 1), "Money", "Money brings happiness");
	...
	public void setKit(Player p, String kitname) {
		...
		if (kit == "") {
			plugin.cm.SendMessage(p, ChatColor.RED + "There's no kit named " + kitname);
			return;
		}
		...
		if (plugin.Game.idle == true) {
			...
			if (kit.startsWith("a") || kit.startsWith("e") || kit.startsWith("i")
					|| kit.startsWith("o") || kit.startsWith("u"))
			...
			if (kit.startsWith("a") || kit.startsWith("e") || kit.startsWith("i")
					|| kit.startsWith("o") || kit.startsWith("u"))
				plugin.cm.SendMessage(p, plugin.trans.get("YouarenowAN").replace("&k", kit));
			else
				plugin.cm.SendMessage(p, plugin.trans.get("YouarenowA").replace("&k", kit));
			if (!p.isDead())
...
//			p.sendMessage("...");
	...
	public void giveKit (Player p) {
		...
		Wool wool = null;
		if (plugin.Game.FlagAHolder instanceof Player && ((Player)plugin.Game.FlagAHolder).getName().equals(p.getName())){
			hasFlag = true;
			wool = new Wool(DyeColor.RED);
		}
		if (plugin.Game.FlagBHolder instanceof Player && ((Player)plugin.Game.FlagBHolder).getName().equals(p.getName())){
			hasFlag = true;
...
//		Wool wool = new Wool(DyeColor.RED);
		...
		if (!done) {
			...
			plugin.log.warning("|Player " + p.getName() + " should be in class "+cn+", but he isn't!!!|");
			setKit(p, cn);
		}
	...
	public boolean hasperm (Player p, String kit){
		...
		if (plugin.Game.disallowedclasses.contains(kit)){
			p.sendMessage(ChatColor.RED + "This class is not allowed in this map!");
			plugin.cm.SendMessage(p, ChatColor.RED + "This class is not allowed in this map!");
			plugin.log.info("Player " + p.getName() + " tries to join a disallowed class(" + kit + ")");
			return false;
		}
