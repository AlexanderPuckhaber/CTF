...
public class AddonManager {
	...
	public AddonManager (CTF pl){
		plugin = pl;
		File dir = new File(this.plugin.getDataFolder() + File.separator + "addons");
		if (!dir.isDirectory())
			dir.mkdirs();
		...
		if (!f.exists()){
			try{
				...
				plugin.log.info("Error while coping the default AnimationManager");
				e.printStackTrace();
			}
		...
		try {
			...
			Class<?> aclass = cl.loadClass(f.getName().substring(0,f.getName().lastIndexOf(".")));		
			Object object = aclass.newInstance();
			if (!(object instanceof AnimationManagerTemplate)) {
				plugin.log.warning("Not a valid AnimationManager: " + aclass.getSimpleName());
				...
				try{
					...
					cl =  new URLClassLoader(new URL[] { f.toURI().toURL() },
							AnimationManagerTemplate.class.getClassLoader());
						aclass = cl.loadClass(f.getName().substring(0,f.getName().lastIndexOf(".")));		
						object = aclass.newInstance();
						if (!(object instanceof AnimationManagerTemplate)) {
							plugin.log.warning("Can't load the default animation Manager! - Something went terrible wrong!");
						}
				} catch (IOException e) {
					plugin.log.info("Error while coping the default AnimationManager b/c yours is corrupted!");
					e.printStackTrace();
				}
			}
		...
		if (!subs.isDirectory())
			subs.mkdirs();
		this.Addons.clear();
		this.Addons.addAll(new AddonLoader(this.plugin).load(subs.getAbsolutePath()));
		...
		for ( Addon a : this.Addons){
			...
			plugin.log.info("Enabled Addon: " + a.getName() + " made by " + authors);			
		}
	}
