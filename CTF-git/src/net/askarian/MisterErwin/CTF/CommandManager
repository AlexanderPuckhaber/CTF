  ...
  implements CommandExecutor
{
  ...
  {
    if (!(sender instanceof Player) && !(cmd.getName().equalsIgnoreCase("ctf")) && args.length > 0 && args[1] == "start") {
...
////        	plugin.cm.changemode ((Player)sender, "classes");     
    	...
    	if (!plugin.Game.after){
    		if (plugin.tm.isinGame((Player) sender))
    			plugin.cm.SendMessage((Player) sender, ChatColor.RED + "This is not the time to vote");
    		return true;
    	}
    	if (!plugin.tm.isinGame((Player) sender))	
    		((Player)sender).sendMessage(ChatColor.RED + "You are not in the Game => You can't vote!!!");
...
//        }
    ...
    if (cmd.getName().equalsIgnoreCase("CTF")) {
      if (args.length == 0) {
        ...
        sender.sendMessage(ChatColor.AQUA + " /CTF join       -  join the game");
        sender.sendMessage(ChatColor.YELLOW + " /CTF leave      -  leave the game");

        if (sender.hasPermission("CTF.admin")) {
          sender.sendMessage(ChatColor.RED + "~~~~~Admin Commands~~~~~");
          sender.sendMessage(ChatColor.AQUA + " /CTF start      -  loads first Map and start Game");
          sender.sendMessage(ChatColor.YELLOW + " /CTF stop      -  stops the game");
          sender.sendMessage(ChatColor.AQUA + " /CTF next       -  loads next Map and starts the Game");
          sender.sendMessage(ChatColor.YELLOW + " /CTF tp         -  teleports into World");
          sender.sendMessage(ChatColor.BLUE + " /CTF set Spawn  -  defines the Spawn for the world");
          sender.sendMessage(ChatColor.GREEN + " /CTF set SpawnA -  defines the Spawn for team A");
          sender.sendMessage(ChatColor.BLUE + " /CTF set SpawnB -  defines the Spawn for team B");
          sender.sendMessage(ChatColor.GREEN + " /CTF set FlagA  -  defines the Flag for team A");
          sender.sendMessage(ChatColor.BLUE + " /CTF set FlagB  -  defines the Flag for team B");
        }
      }
      else if (args[0].equalsIgnoreCase("join")) {
    	  Bukkit.dispatchCommand(sender, "class");
//        if (((!this.plugin.Game.running) && (!this.plugin.Game.idle)) || (this.plugin.Game.Spawn == null || this.plugin.Game.Spawn.getWorld() == null)) {
//          sender.sendMessage(ChatColor.RED + "No Game is Running!");
//          return true;
//        }
//        if (this.plugin.Game.idle) {
//          if (!this.plugin.Game.waitingPlayers.containsKey(sender.getName()));
//          this.plugin.Game.waitingPlayers.put(sender.getName(), "heavy");
//        } else {
...
//        }this.plugin.Game.tp((Player)sender);
      ...
      else if ((args[0].equalsIgnoreCase("start")) && (sender.hasPermission("CTF.admin"))) {
        start(sender);
      } else if ((args[0].equalsIgnoreCase("stop")) && (sender.hasPermission("CTF.admin"))) {
        stop(sender);
      } else if ((args[0].equalsIgnoreCase("next")) && (sender.hasPermission("CTF.admin"))) {
        if (this.plugin.Game.Spawn == null || this.plugin.Game.Spawn.getWorld() == null) {
          sender.sendMessage(ChatColor.RED + "No Map is loaded - use /CTF start");
          return true;
        }
        next(sender);
      } else if ((args[0].equalsIgnoreCase("tp")) && (sender.hasPermission("CTF.admin"))) {
        if (this.plugin.Game.Spawn == null || this.plugin.Game.Spawn.getWorld() == null) {
          sender.sendMessage(ChatColor.RED + "No Map is loaded");
          return true;
        }
        if (this.plugin.Game.FlagA == null || this.plugin.Game.FlagA.getWorld() == null 
        		|| this.plugin.Game.FlagB == null || this.plugin.Game.FlagB.getWorld() == null ){
        	plugin.clm.setKit((Player) sender, plugin.clm.Classes.get(0).getCommand());
        }
        this.plugin.Game.tp((Player)sender);
      } else if ((args[0].equalsIgnoreCase("set")) && (sender.hasPermission("CTF.admin"))) {
        if (this.plugin.Game.Spawn == null || this.plugin.Game.Spawn.getWorld() == null) {
          sender.sendMessage(ChatColor.RED + "No Map is loaded");
          return true;
        }
        if (args.length > 1)
          if (args[1].equalsIgnoreCase("spawn")) {
            sender.sendMessage(ChatColor.BLUE + "Spawn Location set!");
            this.plugin.Game.saveSpawnLocation(((Player)sender).getLocation());
          } else if (args[1].equalsIgnoreCase("spawnA")) {
            sender.sendMessage(ChatColor.BLUE + "Spawn Location A set!");
            this.plugin.Game.saveSpawnLocationA(((Player)sender).getLocation());
          } else if (args[1].equalsIgnoreCase("spawnB")) {
            sender.sendMessage(ChatColor.BLUE + "Spawn Location B set!");
            this.plugin.Game.saveSpawnLocationB(((Player)sender).getLocation());
            ...
            sender.sendMessage(ChatColor.RED + "User /ctf set Spawn or SpawnA or SpawnB");
          }
      } else if ((args[0].equalsIgnoreCase("save")) && (sender.hasPermission("CTF.admin"))) {
        if (this.plugin.Game.Spawn == null || this.plugin.Game.Spawn.getWorld() == null) {
          sender.sendMessage(ChatColor.RED + "No Map is loaded");
          return true;
        }
...
//    List players = this.plugin.getServer().getWorld(this.plugin.Game.world.getName()).getPlayers();
    stop(false);
...
public void stop(boolean next){
	  if (this.plugin.Game.Spawn != null){
		  ...
		  for (Player worldPlayers : this.plugin.Game.Spawn.getWorld().getPlayers()) {
			  if (!next)
				  plugin.tm.LeaveTeam(worldPlayers, false);
			  plugin.Game.PlayerStorage.put(worldPlayers.getName(), true);
			  worldPlayers.teleport(((World)this.plugin.getServer().getWorlds().get(0)).getSpawnLocation(), PlayerTeleportEvent.TeleportCause.PLUGIN);
	  ...
	  for (World w :this.plugin.getServer().getWorlds()){
		  if (w.getName().startsWith("CTF-MAP_")){
			  ...
			  for (Player wp : w.getPlayers()){
			      wp.teleport(((World)this.plugin.getServer().getWorlds().get(0)).getSpawnLocation(), PlayerTeleportEvent.TeleportCause.PLUGIN);
  ...
  private void start() {
	...
	while (wlist.hasNext() ) {
	    ...
	    p.sendMessage(ChatColor.DARK_PURPLE + "You are ported in the new Game!");
	    p.playSound(p.getEyeLocation(), Sound.AMBIENCE_THUNDER, 1, 1);
	}
  ...
  public void changeclass (Player p, String kit){
	  ...
	  if(!plugin.Game.idle && !plugin.Game.running){
		  if (Boolean.valueOf(plugin.conm.get("auto")) == true) {
