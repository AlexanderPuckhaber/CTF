  ...
  private CTF plugin;
	...
	public ConfigManager(CTF pl) {
		this.plugin = pl;
		 ...
		 this.broadcasts.add(
		 "Archer can give instakills when they 20blocks away player!");
...
//			ret.replaceAll("a", ChatColor.)
	...
	public void load() {
		...
		try {
			...
			Iterator<Entry<String, String>> itr = ((HashMap<String, String>) registred.clone()).entrySet().iterator();
			...
			while (itr.hasNext()){
				...
				if (!ll.containsKey(itlr.getKey())){
					...
					plugin.log.info(itlr.getKey() + "<========== is there! (" + ll.get(itlr.getKey()));
			}
	...
	@SuppressWarnings("unchecked")
	private void write(File f, HashMap<String, String> hm, String sep) throws IOException{
		f.delete();
		...
		Iterator<Entry<String, String>> it = ((HashMap<String, String>) hm.clone()).entrySet().iterator();
