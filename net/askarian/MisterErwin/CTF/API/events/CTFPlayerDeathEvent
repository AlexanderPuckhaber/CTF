...
public class CTFPlayerDeathEvent extends Event
{
  ...
  public CTFPlayerDeathEvent(Player player, translator trans)
  {
    ...
    this.player = player;
    this.message4Killer = trans.get("youkilled").replace("&P", player.getName());
    this.killer = (player.getKiller() != null)? player.getKiller() : player;
    this.message4Player = trans.get("killedby").replace("&P", killer.getName());
    
    if (this.killer.getName().equals(this.player.getName()))
    	this.message4Player = trans.get("killedyourself").replace("&P", player.getPlayer().getName());
  }
  ...
  public void setMessage4Player(String m){
	  ...
	  this.getPlayer().getServer().getConsoleSender().sendMessage(ChatColor.AQUA + this.getMessage4Player());
	  if (this.message4Player != m)
		  this.getPlayer().getServer().getConsoleSender().sendMessage(ChatColor.RED + "Can't set the custom message: " + m + " instead " + this.message4Player);
  }
